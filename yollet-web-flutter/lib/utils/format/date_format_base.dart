...
///
/// Example:
///     developer.log(formatDate(DateTime(1989, 02, 1, 5), [am]));
///     // => AM
///     developer.log(formatDate(DateTime(1989, 02, 1, 15), [am]));
///     // => PM
const String am = 'am';
...
/// initialized.
String datetimeFormat(String stringDate, {bool? onlyDate = false}) {
  if (stringDate == "null") {
    return stringDate;
  }
  if (!onlyDate!) {
    final date = convertToDatetime(stringDate);

    return '${date.year}.${date.month}.${date.day}\n${_pad(date.hour)}:${_pad(date.minute)}:${_pad(date.second)}';
  } else {
    final date = convertToDatetime(stringDate, onlyDate: onlyDate).toUtc();
    return '${date.year}.${date.month}.${date.day}';
  }
}

DateTime convertToDatetime(String stringDate, {bool? onlyDate = false}) {
  if (!onlyDate!) {
    final parsedYear = stringDate.toString().substring(0, 4);
    final parsedMonth = stringDate.toString().substring(4, 6);
    final parsedDay = stringDate.toString().substring(6, 8);
    final parsedHour = stringDate.toString().substring(8, 10);
    final parsedMin = stringDate.toString().substring(10, 12);
    final parsedSec = stringDate.toString().substring(12, 14);
    DateTime date = DateTime(
        int.parse(parsedYear),
        int.parse(parsedMonth),
        int.parse(parsedDay),
        int.parse(parsedHour),
        int.parse(parsedMin),
        int.parse(parsedSec));
