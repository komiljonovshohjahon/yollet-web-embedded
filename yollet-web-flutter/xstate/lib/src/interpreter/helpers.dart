...
void mainEventLoop(InterpreterGlobals globals) {
  while (globals.isRunning) {
    ...
    enabledTransitions =
        selectTransitions(externalEvent, globals) as LinkedHashSet<Transition>;
    if (!enabledTransitions.isEmpty) {
      microstep(enabledTransitions.toList(), globals);
    }
  }
...
LinkedHashSet<Transition> selectEventlessTransitions(
  ...
  enabledTransitions = removeConflictingTransitions(enabledTransitions, globals)
      as LinkedHashSet<Transition>;
  ...
  enabledTransitions.forEach((t1) {
    ...
    for (final t2 in filteredTransitions) {
      ...
      if (_intersection.length != 0) {
        if (isDescendant(t1.parent as IState?, t2.parent as IState?)) {
...
void microstep(
  ...
  executeTransitionContent(enabledTransitions as LinkedHashSet<Transition>);
...
void enterStates(
  ...
  computeEntrySet(
    enabledTransitions as LinkedHashSet<Transition?>,
    statesToEnter,
...
State? getTransitionDomain(Transition transition) {
  ...
  if (tstates.length == 0)
    return null;
      ...
      isCompundState(transition.parent as IState?) &&
      tstates.every((s) => isDescendant(s, transition.parent as IState?))) {
    return transition.parent as State?;
  } else {
    return findLCCA([transition.parent, ...tstates]) as State?;
  }
}
...
SCXMLElement? findLCCA(Iterable<SCXMLElement?> stateList) {
  return getProperAncestors(stateList.first, null)
      ...
      .where((x) => stateList
          .skip(1)
          .every((s) => isDescendant(s as IState?, x as IState?)))
      .first;
}
...
SCXMLElement? findOneTarget(SCXMLElement? start, IdRef? target,
  ...
  if (start!.parent != null && start.parent is SCXMLElementWithChildren) {
    ...
    var _found = _parent.children
        ...
        .firstWhere((child) => target!.isRefersTo(child.id!));
...
LinkedHashSet<SCXMLElement?> getProperAncestors(
  ...
  if (isDescendant(state2 as IState?, state1 as IState?))
