...
class Machine<TState> {
  ...
  factory Machine.fromJson(Map data) =>
      buildMachineFromJson(data) as Machine<TState>;
...
extension MachineMethods<TState> on Machine<TState> {
  StateValue<TState?> start<T>({required Iterable<T> initial}) {
    final _initial = initial.firstOrNull ?? this.initial;
    var child = this.states![_initial as TState];
    if (child == null)
      throw new Exception(
  ...
  StateValue<TState?> transition<TEvent, TCurrent>(
    ...
    return _handleEvent(_current, event) as StateValue<TState?>;
  }
  ...
  StateValue _handleEvent<T, TState>(StateValue<TState?> current, T event) {
    ...
    Machine<TState>? lastMachine = this as Machine<TState>?;
    var lastState = 0;
    while (lastMachine != null) {
      machines.add(lastMachine);
      lastMachine = lastMachine.states![states[lastState] as TState]!.child
          as Machine<TState>?;
      lastState++;
    }
